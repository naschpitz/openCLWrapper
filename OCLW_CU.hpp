...
namespace OpenCLWrapper
{
  ...
  class ComputeUnit
  {
    public:
      ...
      void clearKernels();
      std::vector<Kernel> saveKernels();
      void restoreKernels(const std::vector<Kernel>& kernels);
  ...
  template <class T>
  void ComputeUnit::writeBuffer(const std::string& name, const std::vector<T>& hBuffer, ulong dBufferWriteOffset)
  {
    std::map<std::string, cl::Buffer>::const_iterator it = this->dBufferMap.find(name);
    cl::Buffer dBuffer;
    ...
    this->queue.enqueueWriteBuffer(dBuffer, CL_FALSE, sizeof(T) * dBufferWriteOffset, sizeof(T) * hBuffer.size(),
                                   hBuffer.data());
  }
  ...
  template <class T> void ComputeUnit::readBuffer(std::vector<T>& hBuffer)
  {
    ...
    this->queue.enqueueReadBuffer(dBuffer, CL_TRUE, sizeof(T) * bufferOffset, sizeof(T) * count,
                                  hBuffer.data() + bufferOffset);
  }

  template <class T>
  void ComputeUnit::readBuffer(const std::string& name, std::vector<T>& hBuffer, ulong dBufferReadOffset)
  {
    ...
    this->queue.enqueueReadBuffer(dBuffer, CL_TRUE, sizeof(T) * deviceReadOffset, sizeof(T) * count,
                                  hBuffer.data() + hBufferWriteOffset);
  }
